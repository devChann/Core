...
(function (global, factory) {
  ...
  units.ordinal = function () {
    throw new Error(
      "dc.units.ordinal should not be called - it is a placeholder"
    );
  };
  ...
  class BaseMixin {
    ...
    data(callback) {
      ...
      this._data =
        typeof callback === "function" ? callback : utils.constant(callback);
      this.expireCache();
      return this;
    }
    ...
    _computeOrderedGroups(data) {
      ...
      return Array.from(data).sort(
        (a, b) => this._ordering(a) - this._ordering(b)
      );
    }
    ...
    turnOnControls() {
      if (this._root) {
        const attribute = this.controlsUseVisibility()
          ? "visibility"
          : "display";
        this.selectAll(".reset").style(attribute, null);
        this.selectAll(".filter")
          .text(this._filterPrinter(this.filters()))
    ...
    render() {
      ...
      if (this._mandatoryAttributesList) {
        this._mandatoryAttributesList.forEach((e) =>
          this.checkForMandatoryAttributes(e)
        );
      }
  ...
  const ColorMixin = (Base) =>
    class extends Base {
      ...
      calculateColorDomain() {
        const newDomain = [
          d3Array.min(this.data(), this.colorAccessor()),
      ...
      constructor() {
        ...
        this.data((group) => {
          ...
          if (this._sortBubbleSize) {
            ...
            data.sort((a, b) =>
              d3Array.descending(radiusAccessor(a), radiusAccessor(b))
            );
          }
          return data;
        });
  ...
  class CoordinateGridMixin extends ColorMixin(MarginMixin) {
    ...
    _prepareXAxis(g, render) {
      ...
      if (this.isOrdinal()) {
        this._x
          ...
          .paddingOuter(
            this._useOuterPadding() ? this._fOuterRangeBandPadding : 0
          );
      } else {
    ...
    renderXAxis(g) {
      ...
      if (axisXG.empty()) {
        axisXG = g
          ...
          .attr(
            "transform",
            `translate(${this.margins().left},${this._xAxisY()})`
          );
      }
    ...
    _renderVerticalGridLines(g) {
      ...
      if (this._renderVerticalGridLine) {
        ...
        const ticks = this._xAxis.tickValues()
          ? this._xAxis.tickValues()
          : typeof this._x.ticks === "function"
          ? this._x.ticks.apply(this._x, this._xAxis.tickArguments())
          : this._x.domain();
    ...
    xAxisLabel(labelText, padding) {
      ...
      this._xAxisLabelPadding =
        padding === undefined ? DEFAULT_AXIS_LABEL_PADDING : padding;
      this.margins().bottom += this._xAxisLabelPadding;
      return this;
    }
    ...
    renderYAxisLabel(axisClass, text, rotation, labelXPosition) {
      ...
      let axisYLab = this.g().select(
        `text.${Y_AXIS_LABEL_CLASS}.${axisClass}-label`
      );
    ...
    renderYAxis() {
      const axisPosition = this._useRightYAxis
        ? this.width() - this.margins().right
        : this._yAxisX();
      this.renderYAxisAt("y", this._yAxis, axisPosition);
      const labelPosition = this._useRightYAxis
      ...
      if (this._brushOn && this._gBrush) {
        ...
        if (!brushSelection) {
          ...
          const scaledSelection = [
            this._x(brushSelection[0]),
            this._x(brushSelection[1]),
          ];
          ...
          gBrush
            ...
            .attr(
              "transform",
              (d, i) => `translate(${this._x(brushSelection[i])}, 0)`
            )
    ...
    resizeHandlePath(d) {
      ...
      return (
        `M${0.5 * x},${y}A6,6 0 0 ${e} ${6.5 * x},${y + 6}V${
          2 * y - 6
    ...
    _generateClipPath() {
      ...
      utils
        ...
        .attr(
          "transform",
          `translate(-${this._clipPadding}, -${this._clipPadding})`
        );
    }
    ...
    _zoomHandler(newDomain, noRaiseEvents) {
      ...
      if (!noRaiseEvents) {
        if (
          this._rangeChart &&
          !utils.arraysEqual(this.filter(), this._rangeChart.filter())
        ) {
          events.trigger(() => {
    ...
    _onZoom() {
      ...
      if (
        d3Selection.event.sourceEvent.type &&
        ["start", "zoom", "end"].indexOf(d3Selection.event.sourceEvent.type) !==
          -1
      ) {
        return;
      }

      const newDomain = d3Selection.event.transform
        .rescaleX(this._origX)
        .domain();
      ...
      this.on("filtered.dcjs-range-chart", (chart) => {
        if (!chart.filter()) {
          events.trigger(() => {
            this._focusChart
              .x()
              .domain(this._focusChart.xOriginalDomain(), true);
          });
        } else if (
          !utils.arraysEqual(chart.filter(), this._focusChart.filter())
        ) {
          events.trigger(() => {
      ...
      g.each(function (data, index) {
        ...
        const outlierIndices = whiskerIndices
          ? d3Array
              ...
              .concat(d3Array.range(whiskerIndices[1] + 1, n))
          : d3Array.range(n);
        ...
        const pointIndices = d3Array.range(
          whiskerIndices[0],
          whiskerIndices[1] + 1
        );
        ...
        const x0 =
          this.__chart__ ||
          d3Scale.scaleLinear().domain([0, Infinity]).range(x1.range());
  ...
  class StackMixin extends CoordinateGridMixin {
    ...
    yAxisMin() {
      const m = d3Array.min(this._flattenStack(), (p) =>
        p.y < 0 ? p.y + p.y0 : p.y0
      );
    ...
    yAxisMax() {
      const m = d3Array.max(this._flattenStack(), (p) =>
        p.y > 0 ? p.y + p.y0 : p.y0
      );
  ...
  class BarChart extends StackMixin {
    ...
    _renderBars(layer, layerIndex, data) {
      ...
      transition(
        ...
        .attr("fill", pluck("data", this.getColor))
        .select("title")
        .text(pluck("data", this.title(data.name)));
    ...
    extendBrush(brushSelection) {
      if (
        brushSelection &&
        this.round() &&
        (!this._centerBar || this._alwaysUseRounding)
      ) {
        brushSelection[0] = this.round()(brushSelection[0]);
        brushSelection[1] = this.round()(brushSelection[1]);
      }
  ...
  class BoxPlot extends CoordinateGridMixin {
    ...
    boxWidth(boxWidth) {
      ...
      this._boxWidth =
        typeof boxWidth === "function" ? boxWidth : utils.constant(boxWidth);
      return this;
    }
    ...
    plotData() {
      this._calculatedBoxWidth = this._boxWidth(
        this.effectiveWidth(),
        this.xUnitCount()
      );
    ...
    _minDataValue() {
      return d3Array.min(this.data(), (e) =>
        d3Array.min(this.valueAccessor()(e))
      );
    }

    _maxDataValue() {
      return d3Array.max(this.data(), (e) =>
        d3Array.max(this.valueAccessor()(e))
      );
    }

    _yAxisRangeRatio() {
      return (
        (this._maxDataValue() - this._minDataValue()) / this.effectiveHeight()
      );
    }
  ...
  class BubbleChart extends BubbleMixin(CoordinateGridMixin) {
    ...
    plotData() {
      ...
      this.r().range([
        this.MIN_RADIUS,
        this.xAxisLength() * this.maxBubbleRelativeSize(),
      ]);
  ...
  const bubbleChart = (parent, chartGroup) =>
  ...
  class BubbleOverlay extends BubbleMixin(BaseMixin) {
    ...
    _doRender() {
      ...
      this.r().range([
        this.MIN_RADIUS,
        this.width() * this.maxBubbleRelativeSize(),
      ]);
    ...
    _getNodeG(point, data) {
      const bubbleNodeClass = `${BUBBLE_NODE_CLASS} ${utils.nameToId(
        point.name
      )}`;
  ...
  const bubbleOverlay = (parent, chartGroup) =>
      ...
      if (this.hasFilter() && this._multiple) {
        this._cbox
          ...
          .property(
            ...
            (d) =>
              (d &&
                this.filters().indexOf(String(this.keyAccessor()(d))) >= 0) ||
              false
          );
  ...
  class CompositeChart extends CoordinateGridMixin {
    ...
    _prepareYAxis() {
      ...
      if (this._leftYAxisChildren().length > 0 && !this._rightAxisGridLines) {
        this._renderHorizontalGridLinesForAxis(
          this.g(),
          this.y(),
          this.yAxis()
        );
    ...
    _yAxisMax() {
      return utils.add(
        d3Array.max(this._getYAxisMax(this._leftYAxisChildren())),
        this.yAxisPadding()
      );
    }
    ...
    _rightYAxisMax() {
      return utils.add(
        d3Array.max(this._getYAxisMax(this._rightYAxisChildren())),
        this.yAxisPadding()
      );
    }
    ...
    xAxisMin() {
      return utils.subtract(
        d3Array.min(this._getAllXAxisMinFromChildCharts()),
        this.xAxisPadding(),
        this.xAxisPaddingUnit()
      );
    }
    ...
    xAxisMax() {
      return utils.add(
        d3Array.max(this._getAllXAxisMaxFromChildCharts()),
        this.xAxisPadding(),
        this.xAxisPaddingUnit()
      );
    }
  ...
  const compositeChart = (parent, chartGroup) =>
  ...
  class DataGrid extends BaseMixin {
    ...
    constructor(parent, chartGroup) {
      ...
      this._html = function (d) {
        return `you need to provide an html() handling param:  ${JSON.stringify(
          d
        )}`;
      };
      ...
      this._htmlSection = (d) =>
        `<div class='${SECTION_CSS_CLASS}'><h1 class='${LABEL_CSS_CLASS}'>${this.keyAccessor()(
          d
        )}</h1></div>`;
  ...
  class DataTable extends BaseMixin {
    ...
    _doColumnHeaderFormat(d) {
      ...
      return typeof d === "function"
        ? this._doColumnHeaderFnToString(d)
        : typeof d === "string"
        ? this._doColumnHeaderCapitalize(d)
        : String(d.label);
    }
  ...
  class GeoChoroplethChart extends ColorMixin(BaseMixin) {
    ...
    _doRender() {
      ...
      for (
        let layerIndex = 0;
        layerIndex < this._geoJsons.length;
        ++layerIndex
      ) {
    ...
    _generateLayeredData() {
      ...
      for (let i = 0; i < groupAll.length; ++i) {
        data[this.keyAccessor()(groupAll[i])] = this.valueAccessor()(
          groupAll[i]
        );
      }
      return data;
    ...
    _renderRegionG(layerIndex) {
      const regionG = this.svg()
        ...
        .attr("class", (d) => {
          ...
          const regionClass = utils.nameToId(
            this._geoJson(layerIndex).keyAccessor(d)
          );
    ...
    _renderPaths(regionG, layerIndex, data) {
      ...
      transition(
        ...
        this.getColor(data[this._geoJson(layerIndex).keyAccessor(d)], i)
      );
    }
  ...
  const geoChoroplethChart = (parent, chartGroup) =>
  ...
  class HeatMap extends ColorMixin(MarginMixin) {
    ...
    _filterAxis(axis, value) {
      const cellsOnAxis = this.selectAll(".box-group").filter(
        (d) => d.key[axis] === value
      );
      const unfilteredCellsOnAxis = cellsOnAxis.filter(
        (d) => !this.hasFilter(d.key)
      );
      events.trigger(() => {
        const selection = unfilteredCellsOnAxis.empty()
          ? cellsOnAxis
          : unfilteredCellsOnAxis;
        const filtersList = selection
          .data()
          .map((kv) => filters.TwoDimensionalFilter(kv.key));
  ...
  class HtmlLegend {
    ...
    render() {
      this._defaultLegendItemCssClass = this._horizontal
      ...
      const container = this._container
        .append("div")
        .attr("class", this._htmlLegendDivCssClass);
      container.attr(
        "style",
        `max-width:${this._container.nodes()[0].style.width}`
      );
      ...
      const legendItemClassName = this._legendItemClass
        ...
        : this._defaultLegendItemCssClass;
    ...
    _getCurveFactory() {
      ...
      if (this._tension !== null) {
        if (typeof curve.tension !== "function") {
          logger.warn(
            "tension was specified but the curve/interpolate does not support it."
          );
        } else {
...

      {
        ...
        span
          ...
          .tween("text", function () {
            ...
            const interpStart = isFinite(chart._lastValue)
              ? chart._lastValue
              : 0;
            const interp = d3Interpolate.interpolateNumber(
              interpStart || 0,
              newValue
            );
  ...
  const numberDisplay = (parent, chartGroup) =>
  ...
  class PieChart extends CapMixin(ColorMixin(BaseMixin)) {
    ...
    constructor(parent, chartGroup) {
      ...
      this.title(
        (d) => `${this.cappedKeyAccessor(d)}: ${this.cappedValueAccessor(d)}`
      );
    ...
    _drawChart() {
      ...
      this._radius =
        this._givenRadius && this._givenRadius < maxRadius
          ? this._givenRadius
          : maxRadius;
      ...
      if (d3Array.sum(this.data(), (d) => this.cappedValueAccessor(d))) {
        ...
        pieData = pieLayout([
          { key: this._emptyTitle, value: 1, others: [this._emptyTitle] },
        ]);
    ...
    _updateLabelPaths(pieData, arcs) {
      ...
      const arc2 = d3Shape
        .arc()
        .outerRadius(
          this._radius - this._externalRadiusPadding + this._externalLabelRadius
        )
    ...
    _pieLayout() {
      return d3Shape
        .pie()
        .sort(null)
        .value((d) => this.cappedValueAccessor(d));
    }
  ...
  class RowChart extends CapMixin(ColorMixin(MarginMixin)) {
    ...
    _calculateAxisScale() {
      if (!this._x || this._elasticX) {
        const _extent = d3Array.extent(this._rowData, (d) =>
          this.cappedValueAccessor(d)
        );
        if (_extent[0] > 0) {
          _extent[0] = 0;
        }
    ...
    _drawAxis() {
      ...
      transition(axisG, this.transitionDuration(), this.transitionDelay()).call(
        this._xAxis
      );
    }
    ...
    _updateElements(rows) {
      ...
      const rect = rows
        .attr(
          "transform",
          (d, i) => `translate(0,${(i + 1) * this._gap + i * height})`
        )
        .select("rect")
        .attr("height", height)
        .attr("fill", this.getColor)
        ...
        .classed("deselected", (d) =>
          this.hasFilter() ? !this._isSelectedRow(d) : false
        )
        .classed("selected", (d) =>
          this.hasFilter() ? this._isSelectedRow(d) : false
        );
      ...
      transition(rect, this.transitionDuration(), this.transitionDelay())
        .attr("width", (d) =>
          Math.abs(this._rootValue() - this._x(this.cappedValueAccessor(d)))
        )
  ...
  class ScatterPlot extends CoordinateGridMixin {
    ...
    constructor(parent, chartGroup) {
      ...
      this.title(
        (d) =>
          `${this.keyAccessor()(d)},${this.valueAccessor()(
            d
    ...
    _locator(d) {
      return `translate(${this.x()(this.keyAccessor()(d))},${this.y()(
    ...
    _plotOnCanvas(legendHighlightDatum) {
      ...
      data.forEach((d, i) => {
        const isFiltered =
          !this.filter() || this.filter().isFiltered([d.key[0], d.key[1]]);
    ...
    legendHighlight(d) {
      if (this._useCanvas) {
        ...
        this._resizeSymbolsWhere(
          (s) => s.attr("fill") === d.color,
  ...
  const scatterPlot = (parent, chartGroup) =>
  ...
  class SelectMenu extends BaseMixin {
    ...
    size(numberVisible) {
      logger.warnOnce(
        "selectMenu.size is ambiguous - use selectMenu.numberVisible instead"
      );
      if (!arguments.length) {
  ...
  class SeriesChart extends CompositeChart {
    ...
    constructor(parent, chartGroup) {
      ...
      this._keySort = (a, b) =>
        d3Array.ascending(this.keyAccessor()(a), this.keyAccessor()(b));
    ...
    _preprocessData() {
      ...
      const children = nesting.map((sub, i) => {
        const subChart =
          this._charts[sub.key] ||
          this._chartFunction(this, this._chartGroup, sub.key, i);
        ...
        return subChart
          ...
          .group(
            {
              all:
                typeof sub.values === "function"
                  ? sub.values
                  : utils.constant(sub.values),
            },
            sub.key
          )
  ...
  const seriesChart = (parent, chartGroup) =>
  ...
  class SunburstChart extends ColorMixin(BaseMixin) {
    ...
    constructor(parent, chartGroup) {
      ...
      this.title(
        (d) => `${this.keyAccessor()(d)}: ${this._extendedValueAccessor(d)}`
      );
    ...
    _scaleRadius(ringIndex, y) {
      if (ringIndex === 0) {
        ...
        const customRelativeRadius = d3Array.sum(
          this.ringSizes().relativeRingSizes.slice(0, ringIndex)
        );
        const scaleFactor =
          (ringIndex * (1 / this.ringSizes().relativeRingSizes.length)) /
          customRelativeRadius;
        const standardRadius =
          ((y - this.ringSizes().rootOffset) /
            (1 - this.ringSizes().rootOffset)) *
          (this._radius - this._innerRadius);
    ...
    _createSliceNodes(slices) {
      return slices
        ...
        .attr(
          "class",
          (d, i) =>
            `${this._sliceCssClass} _${i} ${this._sliceCssClass}-level-${d.depth}`
        );
    }
    ...
    defaultRingSizes() {
      return {
        ...
        scaleInnerRadius: (d) =>
          d.data.path && d.data.path.length === 1
            ? this._innerRadius
            : Math.sqrt(d.y0),
    ...
    relativeRingSizes(relativeRingSizesFunction) {
      function assertPortionsArray(relativeSizes, numberOfRings) {
        if (!Array.isArray(relativeSizes)) {
          throw new BadArgumentException(
            "relativeRingSizes function must return an array"
          );
        }
      ...
      return {
        partitionDy: () => 1,
        scaleInnerRadius: (d) =>
          this._scaleRadius(d.data.path.length - 1, d.y0),
    ...
    _filtersForPath(path) {
      ...
      for (let i = 0; i < this.filters().length; i++) {
        const currentFilter = this.filters()[i];
        if (
          currentFilter.isFiltered(path) ||
          pathFilter.isFiltered(currentFilter)
        ) {
    ...
    _partitionNodes(data) {
      ...
      const _hierarchy = d3Hierarchy
        .hierarchy(data)
        .sum((d) => (d.children ? 0 : this._extendedValueAccessor(d)))
        .sort((a, b) =>
          d3Array.ascending(
            this.ordering()(getSortable(a)),
            this.ordering()(getSortable(b))
          )
        );

      const _partition = d3Hierarchy
        .partition()
      ...
      const relativeSizes = this.ringSizes().relativeRingSizesFunction(
        _hierarchy.height
      );
    ...
    _tweenSlice(d, element) {
      ...
      element._current = i(0);
      return (t) =>
        this._safeArc(this._buildArcs(), Object.assign({}, d, i(t)));
    }
  }

  const sunburstChart = (parent, chartGroup) =>
  ...
  class TextFilterWidget extends BaseMixin {
    ...
    _doRender() {
      ...
      this._input.on("input", function () {
        chart
          .dimension()
          .filterFunction(chart._filterFunctionFactory(this.value));
        events.trigger(() => {
          redrawAll();
        }, constants.EVENT_DELAY);
      });
  ...
  const textFilterWidget = (parent, chartGroup) =>
